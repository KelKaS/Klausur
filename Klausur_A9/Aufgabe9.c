...
void do_subtrahieren(GtkWidget *subtrahieren, gpointer data){
    int num1 = atoi((char*)gtk_entry_get_text(GTK_ENTRY(number1)));
    int num2 = atoi((char*)gtk_entry_get_text(GTK_ENTRY(number2)));
...
int main(int argc, char **argv) {
    GtkWidget *window, *grid, *calculate, *qs, *subtrahieren;
    gtk_init(&argc, &argv);
    ...
    subtrahieren = gtk_button_new_with_label("subtrahieren"); 
    g_signal_connect(subtrahieren, "clicked", G_CALLBACK(do_subtrahieren), NULL);
    gtk_grid_attach(GTK_GRID(grid), subtrahieren, 3, 0, 1, 1);

    qs = gtk_button_new_with_label("quersumme_der_summe");
    g_signal_connect(qs, "clicked", G_CALLBACK(do_quersumme),NULL);
